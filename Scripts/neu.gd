...
func _physics_process(delta: float) -> void:
	...
	# Handle jump.
	if Input.is_action_just_pressed("Jump") and is_on_floor() && not motion_paused:
		gravity_multiplier = 1
		gravity_double = false
		velocity.y = JUMP_VELOCITY 
		
	# Start falling as soon as jump button is released
	if Input.is_action_just_released("Jump") && velocity.y >0 && not motion_paused:
		velocity.y = 0
